alter table public.user_problems
  ...
  add column if not exists leetcode_frontend_id text,
  add column if not exists leetcode_topics text[] not null default '{}'::text[];
...
begin
  if not exists (
    ...
    where conname = 'user_problems_leetcode_frontend_id_not_blank_check'
      and conrelid = 'public.user_problems'::regclass
  ) then
    alter table public.user_problems
      add constraint user_problems_leetcode_frontend_id_not_blank_check
      check (
        leetcode_frontend_id is null
        or pg_catalog.btrim(leetcode_frontend_id) <> ''
      );
  end if;
end;
$$;

create unique index if not exists user_problems_user_id_leetcode_frontend_id_key
  on public.user_problems (user_id, leetcode_frontend_id)
  where leetcode_frontend_id is not null;

drop function if exists public.create_user_problem_with_timezone(
  text,
  text,
  text,
  text,
...
create function public.create_user_problem_with_timezone(
...
begin
  ...
  begin
    ...
    values (
      ...
      p_title,
      p_difficulty,
      pg_catalog.coalesce(p_leetcode_topics, '{}'::text[]),
      p_notes,
      v_review_stage,
      v_reviewed_at,
      v_next_review_at,
